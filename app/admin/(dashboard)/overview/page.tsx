...
import { AdminPageHeader, AdminStatCard } from "@/components/organisms/admin-page-header";
import { shortId, StatusBadge } from "@/components/organisms/admin-dashboard-ui";
import {
    ...
    return (
      <>
        ...
        <section className="w-full rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900 shadow-sm">
          ...
          <p className="mt-2">
            شغّل ملف <span className="font-semibold">supabase/schema.sql</span> في Supabase SQL Editor لتفعيل الرسوم البيانية، روابط المنتجات النظيفة، ودوال التحليلات.
          </p>
        </section>
      </>
    );
  ...
  return (
    <>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <AdminStatCard icon={<ShoppingBag className="h-4 w-4" />} label="إجمالي الطلبات" value={String(stats.totalOrders)} />
        <AdminStatCard icon={<Truck className="h-4 w-4" />} label="طلبات نشطة" value={String(stats.activeOrders)} />
        <AdminStatCard icon={<BarChart3 className="h-4 w-4" />} label="الإيراد" value={formatPrice(stats.totalRevenue)} />
        <AdminStatCard icon={<Boxes className="h-4 w-4" />} label="المنتجات" value={String(stats.productCount)} note={`متوسط الطلب ${formatPrice(stats.averageOrder)}`} />
      </section>
      ...
      <section className="rounded-2xl border border-rawey-line bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          ...
          <p className="text-xs font-semibold text-rawey-muted">{stats.pendingOrders} قيد المتابعة</p>
        </div>
        <div className="mt-3 divide-y divide-rawey-line">
          {recentOrders.length ? recentOrders.map((order) => (
            <div key={order.id} className="grid gap-2 py-3 text-sm sm:grid-cols-[1fr_auto_auto] sm:items-center">
              <div>
              ...
              <p className="font-semibold sm:text-left">{formatPrice(order.total_price)}</p>
            </div>
...
async function getOverviewDashboard(range: number) {
  try {
    ...
    if (error instanceof Error && /schema cache|function public|get_admin_/i.test(error.message)) {
    ...
    <div className="inline-flex rounded-full border border-rawey-line bg-white p-1 text-xs font-semibold shadow-sm">
      {ADMIN_DASHBOARD_RANGES.map((range) => (
        <Button
          key={range}
          asChild
          href={range === 30 ? "/admin/overview" : `/admin/overview?range=${range}`}

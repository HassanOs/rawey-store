...
export default async function AdminOrdersPage({
  ...
  let orders: PaginatedResult<OrderWithItems> = {
    items: [],
    page,
    pageSize: 10,
    totalCount: 0,
  ...
  try {
    orders = await getAdminOrdersPage(filters, page);
  } catch (error) {
    console.error("Failed to load admin orders page:", error);
  }
      ...
      >
        <div className="grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-[minmax(240px,1.5fr)_minmax(160px,1fr)_minmax(150px,1fr)_minmax(150px,1fr)_minmax(150px,1fr)]">
          ...
          <label className="min-w-0">
            ...
            <Select
              name="status"
              defaultValue={normalizeOrderStatusFilter(filters.status)}
            >
          ...
          <label className="min-w-0">
            ...
            <Select
              name="payment"
              defaultValue={normalizePaymentMethodFilter(filters.paymentMethod)}
            >
